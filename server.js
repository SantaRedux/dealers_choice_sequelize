...
const app = express();
const db = require('./db');
const { models: { Nationality, Painter, Style, Painting } } = db;
...
app.get('/paintings', async(req, res, next) => {
    try {
        ...
        const painters = await paintings.map(painting => Painter.findByPk(painting.painterId).name;
        ...
        res.send(`
                <html>
                    <head></head>
                    <body>
                        <div>${paintings.map((painting, idx) => `<li>${painting.name}, by ${painters[idx]}</li>`).join('')}</div>
                    </body>
                </html>`
        )
    } 
    ...
    try {
        ...
        res.send(`
                <html>
                    <head></head>
                    <body>
                        <div>${painters.map(painter => `<li>${painter.name}</li>`)}</div>
                    </body>
                </html>`
        )
