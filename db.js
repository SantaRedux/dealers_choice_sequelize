...
const { DataTypes: { DATE, STRING, INTEGER, UUID, UUIDV4 } } = Sequelize;
...
const syncAndSeed = async() => {
    ...
    const [italy, spain, france] = await Promise.all( nationality.map(name => Nationality.create({ name })));
    const [picasso, modigliani, monet] = await Promise.all( painters.map( name => Painter.create({ name })));
    const [cubism, expressionism, impressionism] = await Promise.all( styles.map( name => Style.create({ name })));
    ...
    await Promise.all([
        Painting.create({ name: 'Guernica', painterId: picasso.id, styleId: cubism.id}),
        Painting.create({ name: 'Portrait of Jeanne Hebuterne', painterId: modigliani.id, styleId: expressionism.id}),
        Painting.create({ name: "The Artist's Garden at Giverny", painterId: monet.id, styleId: impressionism.id})
    ]);
};
